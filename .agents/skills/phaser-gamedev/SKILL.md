...
name: phaser-gamedev
description: >-
  Build 2D browser games with Phaser 3/4 (JS/TS): scenes, sprites, physics
  (Arcade/Matter), tilemaps (Tiled), animations, input. Covers Phaser 3 and
  Phaser 4 (4.x has API differences -- see references/phaser4-migration.md).
  Trigger: 'Phaser scene', 'Arcade physics', 'tilemap', 'Phaser 3 game',
  'Phaser 4', 'Phaser game'.
---

# Phaser Game Development

Build 2D browser games using Phaser's scene-based architecture and physics
systems. Works with both Phaser 3 and Phaser 4 — when targeting Phaser 4,
read `references/phaser4-migration.md` first for the API differences.
...
| Phaser 4 migration (from Phaser 3) | [phaser4-migration.md](references/phaser4-migration.md) |
...
Phaser provides powerful primitives — scenes, sprites, physics, input — but
**architecture is your responsibility**.

Think in systems: define the scenes, define the entities, define their
interactions — then implement.

**When targeting Phaser 4**, read `references/phaser4-migration.md` first.
Key differences: `Scale.NONE` for pixel art, `scene.make.graphics()` for
procedural textures, gamepad `JustDown` support, and ES module imports from
CDN.
