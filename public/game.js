...
class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    this.titleText = this.add.text(GAME_W/2, 40, 'STELLAR PRINCESSES', { fontSize: '20px', fontFamily: 'monospace', color: '#ffcc33' }).setOrigin(0.5);
    this.add.text(GAME_W/2, 62, '— A Sci-Fi RPG —', { fontSize: '10px', fontFamily: 'monospace', color: '#aa44ff' }).setOrigin(0.5);
    ...
    this.charKeys.forEach((key, i) => {
      const spr = this.add.image(60 + i * 80, this.spriteY, key).setScale(1.5).setOrigin(0.5, 0.8);
      this.titleSprites.push(spr);
    });
    
    // Prompt
    this.promptText = this.add.text(GAME_W/2, 175, 'Press Z / SPACE / ENTER', { fontSize: '11px', fontFamily: 'monospace', color: '#ffffff' }).setOrigin(0.5);
    this.tweens.add({ targets: this.promptText, alpha: 0.2, duration: 800, yoyo: true, repeat: -1 });
    
    // Version
    this.add.text(GAME_W/2, GAME_H - 10, 'v3.5 — Phaser 4.2', { fontSize: '8px', fontFamily: 'monospace', color: '#444466' }).setOrigin(0.5);
  ...
  showMenu() {
    ...
    opts.forEach((o, i) => {
      ...
      const txt = this.add.text(GAME_W/2, 110 + i * 22, (isSel ? '▸ ' : '  ') + o, { fontSize: '12px', fontFamily: 'monospace', color: isSel ? '#ffffff' : '#aaaaaa' }).setOrigin(0.5).setDepth(50);
...
class CombatScene extends Phaser.Scene {
  ...
  renderCombat() {
    ...
    this.combatLog.forEach((msg, i) => {
      this.combatTexts.push(this.add.text(10, startLogY + i * 10, msg, { fontSize: '7px', fontFamily: 'monospace', color: '#cccccc' }).setDepth(302));
    });
    ...
    this.enemies.forEach((e, i) => {
      ...
      this.combatTexts.push(this.add.text(x, y + 18, e.name, { fontSize: '6px', fontFamily: 'monospace', color: '#ff8888' }).setDepth(302).setOrigin(0.5));
    });
    ...
    this.party.forEach((c, i) => {
      ...
      this.combatTexts.push(this.add.text(x, y + 24, c.name, { fontSize: '6px', fontFamily: 'monospace', color: '#44ddff' }).setDepth(302).setOrigin(0.5));
    });
    ...
    if (this.mode === 'main') {
      ...
      this.combatTexts.push(this.add.text(10, GAME_H - 10, 'X/Esc: Back', { fontSize: '7px', fontFamily: 'monospace', color: '#666688' }).setDepth(302));
    }
  }
